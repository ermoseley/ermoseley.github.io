/* ============================================================================
   ...
   is not painted on -- and no grain is ever recycled, so what you are looking
   at is the converged attractor of the drag law rather than a rolling average
   over a population that keeps being reset. See RESEED_RATE.
...
void main(){
  ...
  // Recycling, normally off -- see RESEED_RATE. It is disabled because the
  // clustering is the interesting quantity and it takes many eddy turnovers to
  // converge, so recycling truncates exactly the tail worth looking at.
  //
  // Kept because it is nearly free and it is the escape hatch if a population
  // ever collapses into too few attractors. Note the rebirth is *entrained in
  // the local flow*, not at rest: brightness comes from drift, and a grain at
  // rest in moving gas has the largest drift there is, so reviving at rest made
  // every recycled grain flash on at full brightness before settling.
  if (reseed > 0.0 && hash1(id, uFrame) < reseed) {
...
void main(){
  ...
  // Grain recycling rate, per unit simulation time. Each grain has this
  // probability per unit time of being deleted and reborn somewhere random,
  // entrained in the local flow.
  //
  // OFF. Preferential concentration is the whole point of a polydisperse grain
  // population, and it is a statistic that takes many eddy turnovers to build:
  // continuously re-randomising a fraction of the population truncates exactly
  // the tail you want to look at. With this at zero the clustering you see is
  // the real, converged attractor of the drag law -- every grain has been in the
  // flow since the page loaded, and nothing resets its history.
  //
  // It cost nothing to leave in, so it is still here. Set it to 0.06 to restore
  // the previous behaviour (a mean grain lifetime of ~17 s of simulation time,
  // which at the one-tenth clock is a few minutes of wall time). The reason it
  // ever existed is the failure mode to watch for: with no recycling at all,
  // heavy grains can in principle pile into a few attractors and leave the rest
  // of the frame bare. In practice the stirrers wander, so the attractors move
  // and the population keeps redistributing -- but if a chapter ever does look
  // voided out, this is the dial.
  const RESEED_RATE = 0.0;
  ...
  function Field(canvas) {
    ...
    function step(dt) {
      ...
      gl.uniform1f(P.pupdate.u.reseed, RESEED_RATE * dt);
    ...
    // Every rate-like term (driving, Brownian kicks, recycling) is scaled by the
